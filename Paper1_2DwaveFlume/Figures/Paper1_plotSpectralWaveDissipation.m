...
for i = 1:length(modelInfo.scenarioNumber)
    ...
    cutoff = find(f<0.33);
    D = abs(F1(cutoff)-F2(cutoff))/(66-31);
    sp(1) = subplot(121);
    pp1(i) = plot(f(cutoff),D,'-','linewidth',1.5,'color',cc(i,:));hold on;
end
...
for i = 1:length(modelInfo.scenarioNumber)
    ...
    sp(2) = subplot(122);
    cutoff = find(f<0.33);
    D = abs(F1(cutoff)-F2(cutoff))/(63-31);
    pp2(i) = plot(f(cutoff),D,'-','linewidth',1.5,'color',cc(i,:));hold on;
end
%Global Adjustments
set(sp,'yscale','log','xlim',[0 0.3])
