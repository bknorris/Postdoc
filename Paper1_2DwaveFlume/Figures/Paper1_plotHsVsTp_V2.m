...
for i = 1:length(hModel)
    ...
    %Organize plots by water depth!
    modelID = find(LR.h == hModel(i));
    HsNorm1 = mean(LR.Hs(modelID,2:end)./LR.Hs(modelID,1),2);
    HsNorm2 = mean(HR.Hs(modelID,2:end)./HR.Hs(modelID,1),2);
    x = LR.Tp(modelID);
    y = HsNorm1-HsNorm2;
    if i == 1
...
set(sp(2),'xtick',4:4:20,'xlim',[2 22],...
    'ylim',[0.3 1.2],'ytick',0.2:0.2:1.2,...
    'yticklabel',[])
set(sp(3),'xtick',4:4:20,'xlim',[2 22],...
    'ylim',[-0.4 0.5],'ytick',-0.4:0.2:0.5)
%Positoning
set(sp(1),'position',[0.1 0.12 0.25 0.8])
set(sp(2),'position',[0.37 0.12 0.25 0.8])
set(sp(3),'position',[0.73 0.12 0.25 0.8])
set(leg,'position',[0.86 0.18 0.1 0.1])


%Labeling
ylabel(sp(1),'$\langle H_s\rangle / {H_s}_0 \   \mathrm{(-)}$','interpreter','latex')
ylabel(sp(3),'${\langle H_s\rangle / {H_s}_0}_{LR} - {\langle H_s\rangle / {H_s}_0}_{HR} \   \mathrm{(-)}$','interpreter','latex')
xlabel(sp(1),'$T_p \ \mathrm{(s)}$','interpreter','latex')
xlabel(sp(2),'$T_p \ \mathrm{(s)}$','interpreter','latex')
xlabel(sp(3),'$T_p \ \mathrm{(s)}$','interpreter','latex')
title(sp(1),'Low Relief')
title(sp(2),'High Relief')
title(sp(3),'Difference')

prettyfigures('text',11,'labels',12,'box',1,'tickdir','in','tlength',[0.008 0.008],'gcolor',[0 0 0])
 
set(ff,'units','inches','renderer','painters');
pos = get(ff,'Position');
set(ff,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])

print(ff,[workingDir modelName(1:end-4) '_HsVsTp_V2alt3'],'-dpdf','-r0')
