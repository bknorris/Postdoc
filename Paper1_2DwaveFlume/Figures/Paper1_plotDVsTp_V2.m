...
for i = 1:length(hModel)
    hold on
    %Organize plots by water depth!
    modelID1 = find(LR.h == hModel(i));
    modelID2 = find(LR.h == hModel(i));
    x = HR.Tp(modelID2);
    y = LR.eps(modelID1)-HR.eps(modelID2);
    if i == 1
...
ylabel(sp(3),'${\partial F / \partial x}_{LR} - {\partial F / \partial x}_{HR} \quad   \mathrm{(W/m^2)}$','interpreter','latex')
...
print(ff,[workingDir 'Models_allHR_LR_DVsTp_V2'],'-dpdf','-r0')
