...
for i = 1:length(hModel)
    ...
    text(0.78,0.1,sprintf('h = %0.0f m',hModel(i)),'units','normalized');
end
...
for i = 1:length(hModel)
    ...
    %Calculate depth profiles
    for j = 1:length(modelID)
        Umag = squeeze(LR.Umag(:,:,modelID(j)));
...
%Labeling
xlabel(sp1(4),'$\langle \overline{u}\rangle \  \mathrm{(m/s)}$','interpreter','latex')
xlabel(sp2(4),'$\langle \overline{u}\rangle \  \mathrm{(m/s)}$','interpreter','latex')
...
print(ff,[workingDir modelName(1:end-4) '_UmagProfiles'],'-dpdf','-r0')
