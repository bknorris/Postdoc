...
for i = 1:length(modelInfo.scenarioNumber)
    ...
    for j = 1:length(WGorder)-1
        ...
        F1 = trapz(1025*9.81*SSeta(2:cutoff).*Cg(2:cutoff));
        HR.Hs(i,j) = 4*sqrt(sum(SSeta.*mean(diff(f))));
        
        [SSeta,f] = pwelch(FS.(fn{WGorder(j+1)}).eta,[],[],[],8);
        [~,bathyID] = min(abs(bathyX-HR.WGs(j+1)));
        localh = -1*bathyY(bathyID); %local h at wave gauge
        ...
        Cg = n.*c;
        cutoff = find(f>=2,1,'first');
        F2 = trapz(1025*9.81*SSeta(2:cutoff).*Cg(2:cutoff));
        delX = HR.WGs(j+1)-HR.WGs(1);
        Kub = (2*pi*f(2:cutoff))./sinh(kh(2:cutoff)); %eq 13
        Sub = (abs(Kub).^2).*SSeta(2:cutoff); %eq 12
        ub = sqrt(trapz(Sub)*mean(diff(f))); %eq 11 
        fe(j) = (-2*(F2-F1))./(1025*delX*sqrt(2/pi)*ub.*trapz(Sub));
        D(j) = (F2-F1)./delX;
    end
...
for i = 1:length(modelInfo.scenarioNumber)
    ...
    for j = 1:length(WGorder)-1
        %Calculate wave energy dissipation across patch
        [SSeta,f] = pwelch(FS.(fn{WGorder(j)}).eta,[],[],[],8);
        [~,bathyID] = min(abs(bathyX-LR.WGs(j)));
        localh = -1*bathyY(bathyID); %local h at wave gauge
        ...
        cutoff = find(f>=2,1,'first');
        F1 = trapz(1025*9.81*SSeta(2:cutoff).*Cg(2:cutoff));
        LR.Hs(i,j) = 4*sqrt(sum(SSeta.*mean(diff(f))));
        
        [SSeta,f] = pwelch(FS.(fn{WGorder(j+1)}).eta,[],[],[],8);
        [~,bathyID] = min(abs(bathyX-LR.WGs(j+1)));
        localh = -1*bathyY(bathyID); %local h at wave gauge
