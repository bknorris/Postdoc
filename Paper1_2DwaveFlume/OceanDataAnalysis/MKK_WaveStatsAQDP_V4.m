...
% 12/01/2020: added struct fun dissipation rate calcs (Wiles et al., 2006)
% 12/16/2020: added velocity rotations for easier comparison to the model.
% Note, turn these off if you want to plot true velocities!
%
% BKN - USGS PCMSC 2020
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(data.burst) %num bursts
    ...
    for j = 1:length(idx)-1
        ...
        wave.Tz(j,i) = Tz;
        wave.f(:,i) = F;
        wave.Cpp(:,i) = Cpp; %pressure spectrum at height of instrument (zp)
        wave.Spp(:,i) = Spp; %surface pressure spectrum
        ...
        for jj = 1:length(data.depth) %depth bins. NOTE: bottom 2-3 bins are obscured near the bed!
            ...
            wave.Uc(jj,i) = Uc;
            wave.Uw(jj,i) = Uw;
            wave.dirp(jj,i) = DirPeak;
            wave.spreadp(jj,i) = SpreadPeak;
            wave.omegar(jj,i) = omegar;
            wave.ubr(jj,i) = omegar;
            wave.Tr(jj,i) = Tr;
        end
        
        %% UPDATE: Rotate velocities and turbulence into incoming wave direction
        U = data.u_1205(:,idx(j):idx(j+1),i)./100;
        V = data.v_1206(:,idx(j):idx(j+1),i)./100;
        W = data.w_1204(:,idx(j):idx(j+1),i)./100;
        ...
        %% Save out variables to wave structure
        wave.Urot(:,i) = mean(U,2);
        wave.Vrot(:,i) = mean(V,2);
        wave.W(:,i) = mean(W,2);
        wave.epsilon(:,i) = epsilon;
    end
end
