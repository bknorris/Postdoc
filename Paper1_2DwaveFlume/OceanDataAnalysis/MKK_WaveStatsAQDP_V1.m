...
%
% Changelog: 
% 11/16/2020: added 95% confidence intervals to spectra and DOF to
% attributes.
%
% BKN - USGS PCMSC 2020
%
...
disp(['Spectra will be averaged with: ' num2str(round((nwin/swin)*2)) ' Degrees of freedom'])
...
for i = 1:length(data.burst) %num bursts
    ...
    for j = 1:length(ind)
        ...
        P = detrend(P);
        [Cpp,F,Cppx] = pwelch(P,hanning(nwin),swin*0.7,swin,fs,'ConfidenceInterval',0.95);
        ...
        Sppx = Cppx./(attn.^2);                     %Surface elevation CI's
