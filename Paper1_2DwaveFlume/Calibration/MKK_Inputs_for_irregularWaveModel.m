...
ploton = 1; %turns plots on (= 1) or off (= 0)
...
%Settings
m = 640;                            %number of windows to average spectra, output will be m/2
...
Hs = 4*sqrt(sum(Spp(f>0.05&f<1.2).*mean(diff(f)))); %To compare with waveStats
% amp = sqrt(2.*Spp);                   %Amplitude spectrum, Spp = 1/2*amp^2
...
amp = sqrt(2*Spp.*mean(diff(f)));     %Wave amp = sqrt(2*S(fi)*df) [I'm pretty sure this is right now]
Hi = 2.*amp;                          %Wave height = 2*amp (Wiberg & Sherwood 2008)
%Note: use this to recalc Hs from model data! Hs_mod = 2*sqrt(2*sum(Hi.*mean(diff(f))))
...
if ploton == 1
    ...
    export_fig(f1,[figPath 'MKK_C15_burst144_Conditions'],'-png')
    export_fig(f2,[figPath 'MKK_C15_burst144_spectra'],'-png')
end
