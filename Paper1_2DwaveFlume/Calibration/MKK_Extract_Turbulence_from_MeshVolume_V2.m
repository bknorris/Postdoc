...
modelNames = {'Scenario_26';'Scenario_26_TESTlevel2'};
modelLegend = {'SC26_level3';'SC26_level2'};
%'Level7 refinement model [0.3 - 0.002 m]';
RBRbursts = [151 158]; %RBR bursts to analyze (range: 141 - 162)
rampTime = 8; %timeseries will be adjusted by this number of seconds to skip data during 'rampTime'
modelTimeStep = 0.125; %model timestep in seconds
modelDuration = 24; %length of timeseries in seconds
saveFigs = 0; %save figures to disk; 0 is off, 1 is on
saveData = 0; %save out profile data to disk; 0 is off, 1 is on
loX = 66.5; %lower xlim of sampling from dataset
hiX = 74.5; %upper xlim of sampling from dataset
%% Plot the model data
ff(1) = figure(1);
...
%     modelBasePath = ['c:\Users\user\Documents\Models\MKK_Combined\2D\Burst' burstTxt '\TurbulenceRefinementModels\' modelNames{j} '\MKKmodel\postProcessing\bathySample\surface\'];
    modelBasePath = ['c:\Users\user\Documents\Models\Paper1_2DwaveFlume\ModelRuns\HR_Domain\Scenarios\' modelNames{j} '\MKKmodel\postProcessing\bathySample\surface\'];
    ...
    for i = 1:dataLength
        ...
        x = x(cropX);y = y(cropX);z = z(cropX);epsilon = epsilon(cropX);
    ...
    p(j) = plot(smooth(nansum(data.epsilonBin,2),8),nanmean(data.zBin,2).*-1,...
    ...
    if saveData == 1
        ...
        save([dataPath modelNames{j} '_TurbData_V2.mat'],'data','-mat','-v7.3')
    end
    clear data
end
set(gca,'ydir','reverse','ylim',[0.8 2],...
    'xscale','log')
leg = legend(p,modelLegend);
ylabel('\bf\itDepth [m]')
xlabel('\bf\it$|\overline{\epsilon_{tot}}| \quad [W/m^{2}]$','interpreter','latex')
prettyfigures('text',11,'labels',13)
if saveFigs == 1
    export_fig(ff(1),[figPath 'ModelScale_EpsilonSum_Profiles_V2'],'-png','-r600','-nocrop')
end
