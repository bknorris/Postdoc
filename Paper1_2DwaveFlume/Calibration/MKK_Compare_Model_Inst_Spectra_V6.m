...
% modelNames = {'MKK_CombinedHR_V5'};
% modelFigName = 'MKK_CombinedHR_V5';
insts = {'MKK18C1501rbr-b.nc';'MKK18HR101aqdHR-b.nc'};
instName = {'C15';'HR-Aquadopp'};
% legendText = {'Observations';'k-epsilon';'k-omegaSST,nutkWallFunc';'k-omegaSST,nutkRoughWallFunc';'k-omegaSST,nutURoughWallFunc'};
legendText = {'Observations';'k-epsilon, Fine Mesh'};

%Script Settings
InstxLoc = [29 71.3]; %C15, C16, HR AQDP
nfft1 = [128 256]; %spectral window settings for instruments
nfft2 = [300 300]; %spectral window settings for the model
...
for i = 1:length(modelNames)
    ...
    for j = 1:length(insts)
        ...
        if contains(data.Gatts.INST_TYPE,'RBR')
            ...
            if length(RBRbursts) == 1
                idx = find(data.burst == RBRbursts(1));
            else
        ...
        P = reshape(P,m*n,1);         %redefine for spectral calcs
...
set(sp,'xlim',[0 0.33],'ylim',[0 0.12])
...
if saveFigs == 1
    export_fig(ff1,[figPath  modelFigName 'burst' burstTxt '_AQDP_MODEL-spectra_V6'],'-png','-r600','-nocrop')
    export_fig(ff2,[figPath  modelFigName 'burst' burstTxt '_AQDP_MODEL-freeSurf_V6'],'-png','-r600','-nocrop')
end
