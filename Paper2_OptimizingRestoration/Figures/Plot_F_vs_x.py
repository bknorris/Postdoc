...
for i in range(0, len(wave_unq)):
    for j in range(0, len(spce_unq)):
        ...
        wave = data['wef'][scenario[idx]][gauges][:-1]
        # HsNorm = ([x / wave[0] for x in wave])
    
        x = wave_gauges
        y = np.trapz(wave, axis=0)
        
        x_scaled = spce_unq[j] * 36
        leg = f'$\Delta S = {x_scaled:.1f}$'
        ax[2].semilogy(x[1:] + xs[j], y[1:], color=cmap[i][j],
                       marker=markers[j],
                       markersize=size[j],
                       label=leg)
...
for i in range(0, len(wave_unq)):
    for j in range(0, len(spce_unq)):
        ...
        x_scaled = spce_unq[j] * 36
        leg = f'$\Delta S = {x_scaled:.1f}$'
        ax[1].semilogy(x[1:] + xs[j], y[1:], color=cmap[i][j],
                       marker=markers[j],
                       markersize=size[j],
                       label=leg)

# Plot 120 s waves on axes3
wave_gauges = np.linspace(0, 10.4, 10) / 10.4
...
for i in range(0, len(wave_unq)):
    for j in range(0, len(spce_unq)):
        ...
        x_scaled = spce_unq[j] * 36
        leg = f'$\Delta S = {x_scaled:.1f}$'
        ax[0].semilogy(x[1:] + xs[j], y[1:], color=cmap[i][j],
                       marker=markers[j],
                       markersize=size[j],
                       label=leg)
