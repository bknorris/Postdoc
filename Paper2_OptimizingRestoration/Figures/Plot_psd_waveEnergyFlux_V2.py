...
csv_file = 'modelPostProcessing_PowerSpectraF_V2.csv'
version_no = 'V3'
...
axes1 = f1.add_axes([0.1, 0.58, 0.25, 0.35])
axes2 = f1.add_axes([0.1, 0.15, 0.25, 0.35])
axes3 = f1.add_axes([0.39, 0.58, 0.25, 0.35])
axes4 = f1.add_axes([0.39, 0.15, 0.25, 0.35])
axes5 = f1.add_axes([0.68, 0.58, 0.25, 0.35])
axes6 = f1.add_axes([0.68, 0.15, 0.25, 0.35])
ax = [axes1, axes2, axes3, axes4, axes5, axes6]

# Colormaps from ColorBrewer
BuGn = ['#93ccb1', '#1b6539']
OrRd = ['#f5a677', '#a42e1b']
PuBu = ['#99b3d2', '#1d567a']
cmap = [BuGn, BuGn, OrRd, OrRd, PuBu, PuBu]
...
[ax[i].set_ylim(0, 0.2) for i in range(0, 2)]
[ax[i].set_xlim(0, 0.33) for i in range(2, 6)]
[ax[i].set_ylim(0, 0.16) for i in range(2, 6)]
[ax[i].grid(False) for i in range(0, 6)]
...
ax[5].set_xlabel(r'$T_w \quad \mathrm{(s)}$')

ax[0].set_title(r'$H_s = \mathrm{0.15 \ m}, T_w = \mathrm{30 \ s}, 2D$')
ax[1].set_title(r'$H_s = \mathrm{0.15 \ m}, T_w = \mathrm{30 \ s}, 32D$')
ax[2].set_title(r'$H_s = \mathrm{0.15 \ m}, T_w = \mathrm{60 \ s}, 2D$')
ax[3].set_title(r'$H_s = \mathrm{0.15 \ m}, T_w = \mathrm{60 \ s}, 32D$')
ax[4].set_title(r'$H_s = \mathrm{0.15 \ m}, T_w = \mathrm{120 \ s}, 2D$')
ax[5].set_title(r'$H_s = \mathrm{0.15 \ m}, T_w = \mathrm{120 \ s}, 32D$')
...
leg1 = ax[0].legend(h, labels, frameon=False)

handles, s = ax[1].get_legend_handles_labels()
h = [handles[1], handles[0], handles[4], handles[3], handles[6]]
leg1 = ax[1].legend(h, labels, frameon=False)

handles, s = ax[2].get_legend_handles_labels()
h = [handles[1], handles[0], handles[4], handles[3], handles[6]]
leg1 = ax[2].legend(h, labels, frameon=False)

handles, s = ax[3].get_legend_handles_labels()
h = [handles[1], handles[0], handles[4], handles[3], handles[6]]
leg1 = ax[3].legend(h, labels, frameon=False)

handles, s = ax[4].get_legend_handles_labels()
h = [handles[1], handles[0], handles[4], handles[3], handles[6]]
leg1 = ax[4].legend(h, labels, frameon=False)

handles, s = ax[5].get_legend_handles_labels()
h = [handles[1], handles[0], handles[4], handles[3], handles[6]]
leg1 = ax[5].legend(h, labels, frameon=False)
...
# Save figure
if save_figures:
    fname = 'F_PSD_Tw_30_120_Hs_0_15_V3.pdf'
