# -*- coding: utf-8 -*-
"""
Eps(z) vs Z (colored by Tw & D) for Paper2_optimizingRestoration
...
for i in range(0, len(waves)):
    ...
    for j in range(0, len(wave_unq)):
        for k in range(0, len(spce_unq)):
            ...
            yBins = np.flipud(np.linspace(0.027, 0.001, 22) * 36)
            xBins = stats.binned_statistic(y, x, 'median', bins=22)
            
            x_scaled = (spce_unq[k] * 36) / 0.4
            leg = f'${x_scaled:.1f}D$'
            ax[i][j].plot(np.linspace(1e-9, 6e-1, 10), np.ones((10,)) * 0.8, '--', color='k')
...
# Axis scaling
for i in range(0, 3):
    [ax[i][j].set_xlim(1e-9, 1e-2) for j in range(0, 3)]
...
ax[0][0].set_xlabel(r'$\langle \overline{\epsilon} \rangle \quad \mathrm{(W\left/m^2\right.)}$')
ax[0][1].set_xlabel(r'$\langle \overline{\epsilon} \rangle \quad \mathrm{(W\left/m^2\right.)}$')
ax[0][2].set_xlabel(r'$\langle \overline{\epsilon} \rangle \quad \mathrm{(W\left/m^2\right.)}$')
